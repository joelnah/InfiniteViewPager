package nah.prayer.loopingvp.listener;
...
public class NPageChangeListener implements ViewPager.OnPageChangeListener {
    ...
    public NPageChangeListener(ArrayList<ImageView> pointViews, int page_indicatorId[]){
    ...
    public void onPageSelected(int index) {
        for (int i = 0; i < pointViews.size(); i++) {
            pointViews.get(index).setImageResource(page_indicatorId[0]);
            if (index != i) {
                pointViews.get(i).setImageResource(page_indicatorId[1]);
            }
        }
